...
## Phase 3b (full-solution attempt)
- Exact reformulation (THEORY.md 12): every family of covering walks is a formation over Walsh characters; with distinct positions the spread bound is tight, so answer 2 iff a formation exists with sum_{n>T} 2^-k(n) < 1; coverage then forces batch hole filling (round-robin filling gives rate 2^-k per walk and cannot cover).
- Mine lemma: two walks that are translates after their split cannot both cover (the lower one is sent back to max of the common history), so lockstep filling must alternate with mirror (bit-move) phases.
- Experiments (THEORY.md 13; solver/formation_window.py, rectangle_tiling.py, multi_walk_greedy.py): the dip bookkeeping is combinatorially solvable (box tilings for 4 and 8 branches); a greedy with free step choice sustains 2, 4 and 8 covering walks at pairwise distinct positions for 2000-3000 steps (rates 0.58, ~0.3, ~0.12 per step); exhaustive N <= 11 shows early splits with both walks covering 1..N-1.
- Status: answer is 2 modulo an explicit lockstep-plus-mirror schedule with k(n) ~ 2 log2 n; not proved. Phase 4: build and verify that schedule (or refute batch filling).
