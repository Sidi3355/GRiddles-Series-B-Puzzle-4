...
- Reduction used: Ana learns nothing while the game runs, so her strategy is a fixed schedule of pairwise-disjoint guess sets; ℓ*(a) = min ℓ such that a schedule with ≤ ℓ guesses per turn hits every valid walk (hitting set), ℓ*(N) = max over a. Exhaustive over all N! sequences up to N = 8 in seconds.
...
- Data (K=0, M=∞): ℓ*(N, t0=2) = ∞,2,2,1,1,1,1,1 for N=1..8 (worst a for N=3 is the swap gadget 3,1,2 with walks 3,2,4 / 3,4,2 / 3,4,6); ℓ* grows as t0 approaches N (N=7, t0=3..6 → 2,3,7,10) and falls with K (N=7, t0=6, K=0,2,4,6 → 10,4,2,1).
